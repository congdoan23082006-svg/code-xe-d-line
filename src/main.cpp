...
void loop() {
  ...
  if (mode == 1) {
    ...
    // Đếm số lần gặp vạch 180 độ (8 mắt đen = 0b11111111)
    static int count180 = 0;
    static bool is180Triggered = false;
    
    // Khi vừa vào Mode 3, reset biến đếm
    if (calPID == 1 && millis() - lastTime < 10) {
       count180 = 0;
    }
    ...
    if (millis() - lastUpdate > 200) {
      ...
      // Hiển thị số lần gặp vạch 180 độ
      display.setCursor(0, 15);
      display.print("So lan gap 180 do:");
      
      display.setTextSize(3); // Làm số lần thật to ở giữa màn hình
      display.setCursor(45, 35);
      display.printf("%d", count180);
